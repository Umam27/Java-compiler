...
VariableDeclarators: VariableDeclarator {
	// cout<<($1).type<<"-----"<<helpType<<" ";
	($$).type = helpType;
	insertId(($1).str, ($1).type, yylineno, pps);
	pps = 1;
}
		| VariableDeclarators CM VariableDeclarator {
			// cout<<($3).type<<"-----"<<helpType<<" ";
			insertId(($3).str, ($3).type, yylineno, pps);
			pps = 1;
		}
		;
...
MethodDeclaration: MethodHeader MethodBody {
	...
	insertIR("","", "", "ret");
	insertIR("ENDP",string(helperMethodName), "", "");
}
		;
...
MethodDeclarator: IDENT LP {addChildwithName("mth_"+string(($1).str)); helppps = pps; pps = 1;} RP {
	helperMethodName = ($1).str;
	helperInType = TOC("void");
	insertIR("PROC",string(helperMethodName) , "", "");
}
		| IDENT LP {addChildwithName("mth_"+string(($1).str)); helppps = pps; pps = 1;} FormalParameterList RP {
	helperMethodName = ($1).str;
	helperInType = ($4).type;
	insertIR("PROC", string(helperMethodName), "", "");
}
...
ConstructorDeclarator: SimpleName  LP {addChildwithName("constructor"); line.push(yylineno);} RP {
	helperMethodName = ($1).str;
	helperInType = TOC("void");
	insertIR("PROC", helperMethodName, "", "");
}
		| SimpleName  LP {addChildwithName("constructor"); line.push(yylineno);} FormalParameterList RP {
	helperMethodName = ($1).str;
	helperInType = ($4).type;
	insertIR("PROC", helperMethodName, "", "");
		}
...
WhileStatement: WhileStatement_token Statement {
	insertIR("goto","",ITC(helperGoto-2),"");
	fillGoto(helperGoto+1,lineIR());
}
		;
...
for_token3: for_token2 ForUpdate {insertIR("goto","",ITC(helper1 - 2),"");};
...
ClassInstanceCreationExpression: NEW ClassType LP ArgumentList RP {
	...
	$$.type = ($2).type;
	$$.temp = TOC(insertIR("=", ITC(-1*classSize(($2).str)), "new obj incoming", "newvar"));
	insertIR("","malloc","","call");
	while(!args.empty()){
		insertIR("", args.top(),"","pushparam");
		args.pop();
	}
	$$.temp = TOC(insertIR("=","return object reference on heap", "", "newvar"));
	insertIR("",string(($2).str),"","call");
...
ArrayCreationExpression: NEW PrimitiveType DimExprs {
	...
	$$.temp = TOC(insertIR("=", string(($2).type) + "$" + string(($3).temp), "new", "newvar"));
	aceflag = 1;
}
		| NEW PrimitiveType DimExprs Dims {
	$$.type = TOC(string(($2).type) + string(($3).type) + string(($4).type));
	$$.temp = TOC(insertIR("=", string(($2).type) + string(($3).temp) + string(($4).type), "new", "newvar"));
}
...
DimExprs: DimExpr {
	...
	$$.temp = ($1).str;
		...
		| DimExprs DimExpr {
			...
			$$.temp = ITC(stoi(($1).temp) * stoi(($2).str));
		}
		;
...
DimExpr: LB Expression RB {
	...
	arrsize.push_back(string(($2).str));
}
		;
...
FieldAccess: Primary DOT IDENT {
	$$.type = TOC(membLookup(($1).str, ($3).str));
	$$.temp = TOC(insertIR("=",string(($1).str) + "." + string(($3).str),"FA","newvar"));
}
		| THIS DOT IDENT {
	$$.type = TOC(thisLookup(($3).str));
	$$.temp = TOC(insertIR("=","this." + string(($3).str),"","newvar"));		}
        ;
...
ArrayAccess: Name LB Expression RB {
	...
	$$.temp = TOC(insertIR("*",($3).str,arrOffset(($1).str,arridx),"newvar"));
	arridx++;
}
	...
	checkType(($3).type, "int", yylineno);
	$$.temp = TOC(insertIR("*",($3).str,arrOffset(($1).str,arridx),"newvar"));
	$$.temp = TOC(insertIR("+",($1).temp,$$.temp,"newvar"));
	arridx++;
	if(arridx > checkArrSize(($1).str)){
		$$.temp = TOC("#" + string(($1).str)+"["+string(($$).temp)+"]");
		arridx = 1;
	}
}
		;
...
RelationalExpression: ShiftExpression {
	...
	$$.temp = ($1).temp;
		...
		| RelationalExpression LT ShiftExpression {
			checkType(($1).type, "numeric", yylineno);
			$$.type = TOC("boolean");
			$$.temp = TOC(insertIR("<", ($1).temp, ($3).temp, "cmp"));
}
